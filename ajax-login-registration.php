...
if (isset($_POST['action']) && $_POST['action'] == 'registration') {
    ...
    $error_message = '';
    $checkEmailQuery = $conn->prepare("select * from akun where email_id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $error_message = "Email telah terdaftar! Gunakan email lain.";
        echo $error_message;
    } else {
        $insertQuery = $conn->prepare("insert into akun(first_name,last_name,email_id,contact_number,password) values(?,?,?,?,?)");
        $password = md5($password);
        $insertQuery->bind_param("sssss", $first_name, $last_name, $email_id, $contact_number, $password);
        
        if ($insertQuery->execute()) {
            echo "Terimakasih, akun telah terdaftar. Silahkan masuk.";
            exit();
        } else {
            $error_message = "error";
        }
...
if (isset($_POST['action']) && $_POST['action'] == 'login') {
    ...
    $selectQuery = $conn->prepare("select * from akun where email_id = ? and password = ?");
